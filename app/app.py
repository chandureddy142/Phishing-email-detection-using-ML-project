    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(ext_dir):
            for file in files:
                ...
                arcname = os.path.join('PhishGuard-Extension', os.path.relpath(file_path, ext_dir))
                zf.write(file_path, arcname)
    zip_buffer.seek(0)
    return send_file(zip_buffer, as_attachment=True, download_name='PhishGuard-Extension.zip', mimetype='application/zip')
